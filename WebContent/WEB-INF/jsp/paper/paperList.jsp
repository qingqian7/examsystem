...
<div class="editingarea">
    ...
    <div class="c_editview">
        ...
        <div class="btn_left">
            ...
            <span class="btnL"><em id="export" class="btnR" onclick="exportPaper()">导出</em></span>
        </div>
    </div>
</div>
</body>
...
<script>
    ...
    function exportPaper() {
        ...
        $(".checkboxes").each(function (index, domEl) {
            if ($(domEl).prop("checked")) {
                var data = {};
                data['exportId'] = parseInt($.trim($(domEl).val())); // todo 转成整型，方便前面 ID 的 List<Long>
                console.log(data);
                data = $.param(data);
                $.post(
                    "paperExport.action",
                    ...
                    function () {
                        console.log("send successfully...");
                        window.open("http://localhost:8080/examsystem/paperexport.jsp");// todo 异步加载试卷信息，大action load 小action
                    }
                );
